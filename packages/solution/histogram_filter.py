...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    two_L = robot_spec["wheel_baseline"]
    alpha = (2*np.pi)/N_tot
    d_tot_l = R*(left_encoder_ticks*alpha)
    d_tot_r = R*(right_encoder_ticks*alpha)
    v = (d_tot_l + d_tot_r) / 2 
    w = (d_tot_l - d_tot_r) / two_L
...
def generate_measurement_likelihood(segments, road_spec, grid_spec):
    ...
    print("In generate_measurement_likelihood ...")
...
def histogram_update(belief, segments, road_spec, grid_spec):
    ...
    if measurement_likelihood is not None:
        ...
        belief = np.multiply(belief, measurement_likelihood) # combine
        if(np.sum(belief) == 0):
            belief = measurement_likelihood
        else:
            belief = np.divide(belief, np.sum(belief)) # normalize         
